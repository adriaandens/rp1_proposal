...
The scope of this project consists of creating a method that allows multiple URLs to be opened at the same time while still being able to track all further interaction, such as unexpected HTTP requests and other malicious activity, and link them to the original request/URL. To prove that the method is something feasible, we will, when enough time is available, implement a proof of concept of the method.
