...
\begin{itemize}
\item How do browsers implement and expose (API) opening URLs?
\end{itemize}
...
The scope of this project consists of creating an algorithm that allows multiple URLs to be opened at the same time while still being able to track all further interaction, such as extra HTTP requests, and link them to the original request/URL. To proof that the algorithm is something feasible, we will also implement a proof of concept that implements the algorithm.
